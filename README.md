<div align="center">

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Montserrat&weight=700&size=42&duration=1&pause=999999&color=FFFFFF&center=true&vCenter=true&repeat=false&width=800&lines=KEUN-HYUK+JANG" />
</p>

### Building AI systems that perceive, understand, and interact with the physical world.

<br>

[![Email](https://img.shields.io/badge/Email-forren0418%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:forren0418@naver.com)
[![GitHub](https://img.shields.io/badge/GitHub-vio4l8et-8A2BE2?style=for-the-badge&logo=github)](https://github.com/vio4l8et)

</div>

---

## About Me

- B.S. in Artificial Intelligence, Konyang University
- Undergraduate Researcher at MIND Lab
- Interested in Physical AI, Embodied AI, Vision-Language Learning and Computer Vision


## Publications

- **흉부 X-ray 영상 기반 자동 진단 보고서 생성을 위한 R2Ge과 A3Net 모델의 성능 비교 분석에 관한 연구**
  <br> *1저자, KKITS2025(한국지식정보기술학회)*

- **의료 보고서 생성 성능 향상을 위한 llama-2 기반 모델과 transfomer 모델의 비교 분석 연구**
  <br> *1저자, KAICTS2025(한국인공지능융합기술학회)*

- **YOLOv11 기반 화재 다중 분류 탐지 모델 설계 및 성능 평가**
  <br> *1저자, KAICTS2025(한국인공지능융합기술학회)*

- **제로샷 개방형 어휘 분할을 위한 대비적 Top-K 가이던스 기반 후처리 로짓 정제 기법**
  <br> *2저자, KCC2026(한국컴퓨터종합학술대회)*

- **상관관계 기반 파생변수 생성과 AutoML을 활용한 감귤 착과량 예측**
  <br> *3저자, KCSE2025(한국소프트웨어공학학술대회)*


## Projects

- **환자의 자세 변화 감지와 압력 센서 기반 욕창 예방 AI**
  - Pose Estimation + Pressure Sensor
  - Blanket Occlusion Mitigation using GCN
  - Arduino-based Pressure Sensing and Heatmap Visualization

- **YOLOv11 기반 실시간 탐지 및 산불 성장 단계 분류 시스템**
  - Real-time Fire Detection
  - Fire & Smoke Pattern Analysis
  - Five-stage Fire Growth Classification
  
- **의료 영상과 진단 보고서를 활용한 자동 의료 영상 분석 및 리포트 생성 모델**
  - R2Gen vs A3Net
  - Llama-2 vs Transformer
  - Vision-Language Architecture Analysis

- **CTG 후처리 모듈을 활용한 Zero-Shot Open-Vocabulary Segmentation**
  - Open-Vocabulary Segmentation
  - Contrastive Top-K Guidance (CTG)
  - Training-free Logit Refinement


## Awards

- **WCRC2025 쿠팡 물류 로봇 경진대회**
  <br> 대전기계부품연구원장상
- **KKITS2025 한국지식정보기술학회**
  <br> 융복합우수논문상
- **2025 창의혁신 DNA 산학협력 팀 프로젝트**
  <br> 공학혁신상(장려상)
- **2025 건양대학교 창업 아이디어 경진대회**
  <br> 대상


## Skills

<div align="center">

<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white">
<img src="https://img.shields.io/badge/C-A8B9CC?style=flat-square&logo=c&logoColor=white">
<img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white">
<img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white">

</div>
